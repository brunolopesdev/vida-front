...
export const GlobalContextProvider = ({ children }) => {
  ...
  const onSubmit = async (name) => {
    ...
    if (name) {
      ...
      const findUser = data.find(patient => patient.resource.name[0].given[0].toLowerCase() === name.toLowerCase());
  ...
  const getPatientAppointments = async (id) => {
    const { data } = await axios.get(`https://vida-api.vercel.app/patient-appointments/${id}`)
  ...
  return (
    <GlobalContext.Provider value={{ onSubmit, loggedUser, getPatientAppointments, appointments, open, handleModal }}>
      {children}
